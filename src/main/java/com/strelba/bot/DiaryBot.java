...
public class DiaryBot extends TelegramLongPollingBot {
    ...
    private void handleMessage(Message message) {
        ...
        if (step == null) {
            sendText(chatId, "Чтобы начать заполнение дневника, нажмите /start", null);
            return;
        }
    ...
    private void processAdminAction(long chatId, UserSession session, String text) {
        ...
        try {
            ...
            sendText(chatId, "ID должен быть числом. Попробуйте еще раз или нажмите Отмена.",
                    oneButton("Отмена", "ADMIN_CANCEL_ACTION"));
            return;
        }
    ...
    private void sendWelcome(long chatId, long userId) {
        ...
        if (admin) {
            rows.add(List.of(button("Открыть админ-панель", "ADMIN_MENU")));
        }
        InlineKeyboardMarkup markup = rows.isEmpty() ? null : new InlineKeyboardMarkup(rows);
    ...
    private void askCurrentStep(long chatId, UserSession session) {
        ...
        String text = "Вопрос " + step.code() + ": " + step.title();
    ...
    private InlineKeyboardMarkup buildScaleKeyboard(SurveyStep step) {
        ...
        rows.add(List.of(button("Пропустить", "SKIP:" + step.code())));
        return new InlineKeyboardMarkup(rows);
    }

    private InlineKeyboardMarkup skipOnlyKeyboard(SurveyStep step) {
        return oneButton("Пропустить", "SKIP:" + step.code());
    }
    ...
    private void notifyAdmins(long authorUserId, String summary) {
        String msg = "Новая запись в дневнике!\n"
                ...
                + summary;
        InlineKeyboardMarkup kb = oneButton("Открыть админ-панель", "ADMIN_MENU");
    ...
    private String entryText(Map<SurveyStep, String> answers) {
        return "A1 Дата Время: " + safe(answers.get(SurveyStep.A1_DATE_TIME)) + "\n"
                + "B1 Площадка: " + safe(answers.get(SurveyStep.B1_VENUE)) + "\n"
                + "C1 Сон: " + safe(answers.get(SurveyStep.C1_SLEEP)) + "\n"
                + "D1 Самочувствие: " + safe(answers.get(SurveyStep.D1_WELLBEING)) + "\n"
                + "E1 Еда до тренировки: " + safe(answers.get(SurveyStep.E1_FOOD)) + "\n"
                + "F1 Погода: " + safe(answers.get(SurveyStep.F1_WEATHER)) + "\n"
                + "G1 Ветер (м/с): " + safe(answers.get(SurveyStep.G1_WIND)) + "\n"
                + "H1 Освещение: " + safe(answers.get(SurveyStep.H1_LIGHTING)) + "\n"
                + "I1 Упражнение: " + safe(answers.get(SurveyStep.I1_EXERCISE)) + "\n"
                + "J1 Результат: " + safe(answers.get(SurveyStep.J1_RESULT)) + "\n"
                + "K1 Мишень максимум: " + safe(answers.get(SurveyStep.K1_TARGET_MAX)) + "\n"
                + "L1 Комментарий: " + safe(answers.get(SurveyStep.L1_COMMENT)) + "\n"
                + "M1 Город: " + safe(answers.get(SurveyStep.M1_CITY));
    }
    ...
    private void sendResultsPage(long chatId, int page) {
        ...
        if (totalDays == 0) {
            sendText(chatId, "Результатов пока нет.", oneButton("Назад в панель", "ADMIN_MENU"));
            return;
        }
        ...
        if (day == null) {
            sendText(chatId, "Не удалось получить результаты за выбранный день.", oneButton("Назад в панель", "ADMIN_MENU"));
            return;
        }
    ...
    private InlineKeyboardMarkup resultsKeyboard(int page, int total) {
        ...
        if (page > 0) {
            nav.add(button("⬅ День", "ADMIN_RESULTS_PAGE:" + (page - 1)));
        }
        if (page < total - 1) {
            nav.add(button("День ➡", "ADMIN_RESULTS_PAGE:" + (page + 1)));
        }
        ...
        rows.add(List.of(button("Назад в панель", "ADMIN_MENU")));
        return new InlineKeyboardMarkup(rows);
    }
    ...
    private void sendUsersPage(long chatId, int page) {
        ...
        if (totalUsers == 0) {
            sendText(chatId, "Пользователей пока нет.", oneButton("Назад в панель", "ADMIN_MENU"));
            return;
        }
    ...
    private InlineKeyboardMarkup usersKeyboard(int page, int totalPages) {
        ...
        if (page > 0) {
            nav.add(button("⬅ Страница", "ADMIN_USERS_PAGE:" + (page - 1)));
        }
